...
- **Besu 노드**: `http://localhost:8545`에서 실행
- **Geth 노드**: `http://localhost:8445`에서 실행

필요에 따라 둘 중 하나 또는 둘 다 동시에 실행할 수 있습니다.

## 프로덕션 RPC 토폴로지

프로덕션 환경에서의 라우팅, 안정성 및 성능을 위해 아래 토폴로지 가이드를 따르세요.

체인의 변경 관련 흐름에 관여하기 위한 쓰기(write) 가능한 RPC 구성을 위해서는 스테이터스 네트워크에 정적 IP 허용 목록 등록이 필요할 수 있습니다. 관련한 부분은 팀에게 문의 바랍니다.

### 쓰기 가능한 RPC

- **최소 구성**: Besu 클라이언트 1개 실행
- **권장 구성**: Besu + Geth 2개 클라이언트를 동시에 실행
- **Besu 필수 메소드**:
  - `linea_estimateGas`
  ...
  - `eth_gasPrice`
- **라우팅 규칙**: 그 외 모든 메소드는 읽기 성능 최적화를 위해 Geth로 라우팅
...
- `eth_getLogs` (큰 범위): 200ms ~ 수 초. 제한된 범위와 블룸 필터를 사용하세요.
- `eth_estimateGas`: 200ms ~ 수 초. 부하가 높거나 복잡한 컨트랙트의 경우 느려질 수 있습니다.
...
- [네트워크 세부정보](/general-info/network-details) 및 전반적인 문서 검토
