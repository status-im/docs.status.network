...
MetaMask에 커스텀 네트워크로 **Status Network Hoodi 테스트넷**을 추가하는 방법:
...
4. **네트워크 세부 정보 입력**:
   ...
   - 다음 정보를 입력합니다:
     ...
     | **체인 ID**      | 374                              |
     | **통화 기호**    | `ETH`                                   |
     | **블록 탐색기 URL** | https://hoodiscan.status.network |

5. **네트워크 저장**:

   - **"저장"**을 클릭하여 Status Network Hoodi 테스트넷을 MetaMask 지갑에 추가합니다.

이제 네트워크 드롭다운 메뉴에서 Status Network Hoodi 테스트넷을 선택하여 연결할 수 있습니다.
...
- **공식 링크**:
  ...
  - [Status Network Hoodi 테스트넷 탐색기](https://hoodiscan.status.network)
