---
title: 'カルミックトークノミクス：レピュテーションとガバナンス力を獲得'
description: 'カルミックトークノミクスの解説。ソウルバウンドのカルマがトランザクションスループット、ネイティブ利回り配分、ネットワークレベルのスパムスラッシングをどう規定するかを学びます。'
keywords:
  - 'カルマトークン'
  - 'カルミックトークノミクス'
  - 'Status Networkガバナンス'
...
カルマティアは各アドレスのガスレストランザクション割当を決めます。ティアが高いほど、エポックあたりの無料トランザクションが増えます。

### 現在のティアモデル（テストネット）

| ティア | 1日あたりの割当 | 等価レート | Karmaレンジ |
|---|---|---|---|
...
:::info テストネット専用
このティアモデルは現在テストネット向けの整合に使われています。ティアごとの割当数値は、将来カルマ保有者のガバナンスで変更される可能性があります。
:::
